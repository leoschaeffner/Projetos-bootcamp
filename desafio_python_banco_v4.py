...
def log_transacao(func):
    def wrapper(*args, **kwargs):
        ...
        # Executa a função original e captura o resultado
        resultado = func(*args, **kwargs)

        # Cria o log com todas as informações necessárias
        log = f"{data_hora} - {nome_funcao} - {argumentos} - retorno={resultado}\n"

        # Salva no arquivo log.txt (append para não sobrescrever)
...
def criar_conta(agencia, numero_conta, usuarios):
    ...
    if usuario:
        conta = {"agencia": agencia, "numero": numero_conta, "usuario": usuario, "saldo": 0, "extrato": []}
